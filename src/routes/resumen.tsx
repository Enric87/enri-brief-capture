...
function Dato({ label, valor }: { label: string; valor?: string }) {
  ...
  return (
    <div className="border-b border-border py-3 last:border-0">
      ...
      <p className="mt-1 whitespace-pre-line text-sm text-foreground">{valor}</p>
    </div>
  );
}
...
function Bloque({ titulo, children }: { titulo: string; children: React.ReactNode }) {
  return (
    <section className="rounded-2xl border border-border bg-card p-5 shadow-soft">
      <h2 className="mb-2 text-base font-semibold tracking-tight text-foreground">{titulo}</h2>
      <div>{children}</div>
    </section>
  );
}
...
function ResumenPage() {
  ...
  if (!briefing) {
    return (
      <div className="mx-auto flex min-h-screen max-w-2xl flex-col items-center justify-center gap-4 px-6 text-center">
        <h1 className="text-2xl font-semibold text-foreground">Aún no hay briefings guardados</h1>
        <p className="text-sm text-muted-foreground">
          Rellena la ficha y guárdala para verla aquí.
        </p>
        <Link
          to="/"
          className="inline-flex min-h-12 items-center rounded-xl bg-mustard px-5 text-sm font-semibold text-accent-foreground"
        >
          Ir al briefing
        </Link>
      </div>
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      <header className="border-b border-border bg-card/70 px-5 pt-8 pb-7">
        <div className="mx-auto max-w-2xl">
          <Link
            to="/"
            className="inline-flex min-h-11 items-center gap-2 text-sm text-muted-foreground"
          >
            <ArrowLeft className="h-4 w-4" /> Volver al briefing
          </Link>
          <div className="mt-4 flex items-center gap-2 text-accent-foreground">
            <CheckCircle2 className="h-5 w-5" />
            <span className="text-sm font-medium">Briefing guardado correctamente</span>
          </div>
      ...
      <main className="mx-auto max-w-2xl space-y-4 px-5 py-6">
        ...
        <Bloque titulo="Información general">
          ...
          <Dato label="Objetivo principal" valor={b.general.objetivo} />
              ...
              b.referencias
                ...
                .map((r, i) => (
                  <div key={r.id} className="overflow-hidden rounded-xl border border-border">
                    ...
                    <div className="p-3">
                      <Dato label="URL" valor={r.url} />
                      <Dato label="Qué le gusta" valor={r.gusta} />
        ...
        {b.tipos.includes("logo") ? (
          <Bloque titulo="Logo / Branding">
            ...
            <Dato label="Símbolos deseados" valor={b.logo.simbolos} />
            <Dato label="Lo que no quiere" valor={b.logo.noQuiere} />
            <Dato label="Usos" valor={lista(b.logo.usos)} />
          </Bloque>
        ) : null}

        {b.tipos.includes("web") ? (
          <Bloque titulo="Proyecto web">
        ...
        {b.tipos.includes("app") ? (
          <Bloque titulo="Proyecto de app">
            <Dato label="Problema que resuelve" valor={b.app.problema} />
            <Dato label="Usuarios" valor={b.app.usuarios} />
            <Dato label="Flujo principal" valor={b.app.flujo} />
        ...
        {b.tipos.includes("automatizacion") ? (
          <Bloque titulo="Automatización">
            ...
            <Dato label="Tiempo que ocupa" valor={b.automatizacion.tiempo} />
            <Dato label="Frecuencia" valor={b.automatizacion.frecuencia} />
            <Dato label="Quién la realiza" valor={b.automatizacion.quien} />
